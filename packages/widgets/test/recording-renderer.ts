...
export function recordingRenderer(): { r: Renderer; calls: Call[] } {
  const calls: Call[] = [];
  const names = ['resize','beginFrame','endFrame','clear','save','restore','translate','scale','clipRect','setShadow','setGlobalAlpha','setLineDash','fillRect','strokeRect','fillRoundRect','strokeRoundRect','fillPath','strokePath','drawText','drawImage'];
  const r: any = {};
