...
- **Material-кит (`@cairn/material`):** `createMaterialTheme` (palette light/dark, elevation 0–24dp, типографика, shape, spacing) + цветовые хелперы (`lighten`/`darken`/`alpha`/`contrastText`). Компоненты — MT2+. Своя реализация Material-Design-языка (не копия MUI).
