...
- **Material-кит (`@cairn/material`):** `createMaterialTheme` (palette light/dark, elevation 0–24dp, типографика, shape, spacing) + цветовые хелперы (`lighten`/`darken`/`alpha`/`contrastText`). MT2 отгружён: `createRipple` (touch-ripple, расширяющийся/исчезающий круг по `trigger(x,y)`) + `stateLayerOpacity`/`stateOverlay` (слой состояния Material-3). Своя реализация Material-Design-языка (не копия MUI).
