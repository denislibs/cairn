# Canvas Frontend Framework — Дизайн и роадмап
...
SolidJS-подобный фронтенд-фреймворк, который рендерит приложение **на `<canvas>`**, а не в DOM. Свои примитивы, своя реактивность, свой layout, своя стилизация и текстовый ввод.
...
| Примитивы v1 | View/Box, Text, Row, Column |
...
packages/
  ...
  reactivity/    // signals, effects, memo, batch, owner/cleanup
  layout/        // constraints-down/size-up движок (чистая математика)
  runtime/       // дерево элементов, связка реактивность↔дерево, jsx-runtime
  events/        // модель событий: hit-testing, bubbling, focus (без DOM)
  style/         // StyleSheet, состояния, тема
  router/        // сопоставление маршрутов (history-адаптер — из Host)
  primitives/    // View, Text, Row, Column (+ Button/Input/…)
  # --- ПЛАТФОРМЕННЫЙ ШОВ ---
  host/          // интерфейсы Host: Renderer, InputSource, TextInputService,
                 //   AccessibilityBridge, FrameScheduler, SurfaceMetrics
  platform-web/  // веб-реализация Host: Canvas2DRenderer, DOM-события,
                 //   скрытый input-прокси, DOM a11y-зеркало, rAF, ResizeObserver
  # --- ПРИЛОЖЕНИЕ ---
  app/           // createRoot(host), mount
examples/
  counter/
```
...
(`jsx`/`jsxs`/`Fragment`) + Vite `jsxImportSource`; примитивы **View, Text, Row, Column**;
...
### Фаза 5 — Система стилей `style` 🔗 4
📦 `StyleSheet.create`, разрешение стилей в layout+paint свойства, состояния
(`hover/focus/active/disabled/pressed`), токены темы + `ThemeProvider`, каскад/мерж, единицы.
✅ Тесты разрешения стилей/состояний; пример с темизированными карточками.

### Фаза 6 — События `events` + `InputSource` (web) 🔗 4
...
### Фаза 7 — Текстовый ввод: `TextInputService` (web) + Input 🔗 5,6
...
### Фаза 8 — Control flow и списки 🔗 4
...
### Фаза 9 — Расширенные примитивы 🔗 6,7,8
...
### Фаза 11 — Производительность 🔗 всё
...
### Фаза 14 — DevTools и отладка 🔗 всё
...
### Фаза 15 — Роутинг `router` 🔗 1,8
📦 `<Router>` (history/hash), `<Route path>` с параметрами (`/user/:id`), вложенные маршруты +
`<Outlet>`, `<Link>`/`navigate()`, `useParams`/`useSearchParams`, ленивая загрузка, скролл-восстановление.
✅ Приложение с 2–3 экранами, deep-link по URL, корректные back/forward.

### Фаза 16 — Production-hardening 🔗 всё
...
### Фаза 17 — Релиз и DX 🔗 всё
📦 Сборка пакетов (ESM+типы, tree-shaking, size-budget), semver + changesets, документация
(гайды + API + рецепты), `create-canvas-app`, плейграунд, contributing-гайд.
...
- **После M3 параллелятся:** стили (5), события (6), control-flow (8).
- **Сквозные (закладывать интерфейсы рано, наполнять в свою фазу):** интерфейсы `Host`
  (Фаза 2), a11y-`AccessibilityBridge` (13), dirty-region (11).
