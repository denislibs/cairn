...
packages/
  ...
  renderer/      // Renderer-интерфейс + canvas2d-бэкенд
  layout/        // constraints-down/size-up движок
  runtime/       // дерево элементов, связка реактивность↔дерево, jsx-runtime
  events/        // hit-testing, pointer, focus, клавиатура
  input-proxy/   // скрытый DOM-input
  style/         // StyleSheet, состояния, тема
  router/        // history/hash-роутинг (надстройка)
  primitives/    // View, Text, Row, Column (+ Button/Input/…)
  app/           // createRoot(canvas), mount
examples/
  counter/
```
...
### Фаза 2 — Renderer `renderer` 🔗 —
📦 Интерфейс `Renderer` (fillRect, roundRect, fillText, drawImage, clip, save/restore, HiDPI),
бэкенд `Canvas2DRenderer`, управление контекстом и ресайзом.
✅ Тесты через мок-контекст; пример со статичными фигурами/текстом, чёткий HiDPI.
...
📦 `runtime`: дерево элементов над layout-узлами; реактивная привязка (эффект→dirty→scheduler
на rAF→точечный re-layout→paint региона); `jsx-runtime` (`jsx`/`jsxs`/`Fragment`) + Vite
`jsxImportSource`; примитивы **View, Text, Row, Column**; `createRoot(canvas)` + `mount`.
...
### Фаза 6 — События `events` 🔗 4
📦 Hit-testing (обход дерева с z/clip), нормализация pointer (mouse+touch), bubbling/capture,
...
### Фаза 7 — Текстовый ввод `input-proxy` + Input 🔗 5,6
📦 Скрытый DOM `<input>/<textarea>`-прокси, синхронизация значения ↔ signal, отрисовка
поля/курсора/выделения, позиционирование прокси под фокус, IME/composition, буфер, мобильная клава.
...
### Фаза 13 — Доступность (a11y) 🔗 6,9
📦 Скрытое DOM-зеркало дерева (ARIA-узлы) для скринридеров, роли/лейблы, клавиатурная навигация,
объявления. Интерфейс зеркала закладывается рано, наполняется здесь.
...
- **Сквозные (закладывать интерфейсы рано, наполнять в свою фазу):** a11y-зеркало (14),
  dirty-region (12).
