...
**Дата:** 2026-07-05
**Статус:** реализовано (DT3: имена + style + why-frame + профайлер); дельта-протокол вынесен в D4
...
- **Дельта-протокол снапшотов** — вынесен из DT3 в D4 (чистое ядро было построено и удалено: неиспользуемо в DT3 + не обрабатывало reparenting/move; в D4 сделать с move-детекцией и подключить к агенту+панели).
